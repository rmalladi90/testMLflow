...
for word in words_review:
    try:
        if (
            word_index[word] > 10000
        ):  # remember we trained model to consider only 10000 high rated words
            indx = 2
        else:
            indx = word_index[word]
    except:  # noqa  TODO(rmalladi90): See if we can make this more narrow
        indx = 2
    imds_index.append(
        indx
    )  # if a word is not in the word index I'm assigning 2 (OOV index according to documentation)

print(imds_index)


max_len = (
    300  # Only consider the first 300 words of each movie review. If it is less pad it
)
