...
print(
    f"Number of unique words in training dataset: {len(np.unique(np.hstack(X_train)))}"
)

print(f"Number of rows in validation dataset: {X_valid.shape[0]}")
print(
    f"Number of unique words in validation dataset: {len(np.unique(np.hstack(X_valid)))}"
)
