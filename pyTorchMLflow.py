...
vocab = {word: i+2 for i, (word, _) in enumerate(word_counts.most_common(hyperparams["vocab_size"]-2))}
vocab["<PAD>"] = 0
vocab["<UNK>"] = 1
...
X = np.array([encode_and_pad(r, vocab, hyperparams["max_length"]) for r in df['review']])
#y = df['sentiment']
...
with mlflow.start_run():
    ...
    model = SentimentLSTM(hyperparams["vocab_size"], hyperparams["embedding_dim"], hyperparams["hidden_dim"]).to(device)
    ...
    for epoch in range(hyperparams["epochs"]):
        ...
        print(f"Epoch {epoch+1}/{hyperparams['epochs']} -> Val Loss: {epoch_val_loss:.4f}, Val Acc: {epoch_val_acc:.4f}")
